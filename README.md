...
2. SEO問題の解消<br>
    Next.jsを例に挙げるとSSR及びSSGはPre-rendering機能[^6]を有していて、そのHTMLがブラウザに表示されると、Javascriptが読み込まれ、Reactコンポーネントが有効になるが、その際にaタグをラップするlinkタグが含まれてるため、このaたぐによって、Googleクローラーはリンクをたどることが出来る。
